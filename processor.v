...
wire bnpos_cond;        // OR kapisinin cikisi
wire do_bnpos_branch;   // AND kapisinin cikisi
...
// J-Tipi Adres Hesaplama (Shift modulu yerine dogrudan kablo birlestirme ile sifizirsiz 32-bit uretilir)
...
// Control unit: En sona bnpos KABLOSU EKLENDI!
control cont(
    instruc[31:26], 
    instruc[5:0], 
...
// OR KAPISI: Status[Z] VEYA Status[N]
assign bnpos_cond = status[0] | status[1]; 

// AND KAPISI: "Komut bnpos mu?" VE "Kosul (OR) 1 mi?"
...
initial 
begin
$monitor($time," PC %h  INST %h  STAT=%b  do_j=%b  MEM[8]=%h  MEM[12]=%h", 
